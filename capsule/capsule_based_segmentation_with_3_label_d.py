...
with tf.Session() as sess:
    ...
    loss_vals = []
    training_step = 0
    for train_index in range(len(train_dir)):
        ...
        for i in range(target[0], X_feeder_raw.shape[0]-target[0]-1):
            for j in range(target[1], X_feeder_raw.shape[1]-target[1]-1):
                X_feeder = X_feeder_raw[i-seg_width//2 : i+seg_width//2+1, j-seg_height//2 : j+seg_height//2+1,
                        :].reshape(1, seg_width, seg_height, 3)
                y_feeder = y_feeder_raw[i][j].reshape([1])
